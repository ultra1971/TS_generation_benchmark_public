...
#     assert anomaly_type in ['extreme', 'shift', 'trend', 'variance']
    if anomaly_type == 'extreme':
        ...
        raise ValueError('Anomaly type should be one between [\'extreme\', \'shift\', \'trend\', \'variance\']')
...
#    save_path_parameters = f'../../results/AnomaliesInjection/{name}/{name}_parameters.txt'
    ...
    save_path_plots = f'../../results/{name}/plots/{name}_plots.pdf'
#    save_path_plots = None
    ...
    if save_path_results != None:
        pd.concat([copy,df], axis=1).to_csv(save_path_results, header=True, index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default="../../datasets/Original_Data/currencies3.csv", help='Path to the input dataset')
    parser.add_argument('--nb_modifications', type=int, default=1000, help='Number of modifications. If -1, an avarage of 1 modification per time series is used')
    ...
    parser.add_argument('--seed', type=int, default=0, help='Seed determining the random generation')
    parser.add_argument('--max_nb_extreme', type=int, default=20, help='Max number of extreme values (spikes) per anomaly')
    parser.add_argument('--min_shift', type=int, default=1, help='Min length of each shift anomaly')
    parser.add_argument('--max_shift', type=int, default=5, help='Max length of each shift anomaly')
    parser.add_argument('--min_trend', type=int, default=1, help='Min length of each trend anomaly')
    parser.add_argument('--max_trend', type=int, default=10, help='Max length of each trend anomaly')
    parser.add_argument('--min_variance', type=int, default=1, help='Min length of each variance anomaly')
    parser.add_argument('--max_variance', type=int, default=20, help='Max length of each variance anomaly')
    parser.add_argument('--extreme_factor', type=int, default=1, help='Max intensity of each extreme anomaly')
    parser.add_argument('--shift_factor', type=int, default=1, help='Max intensity of each shift anomaly')
    parser.add_argument('--trend_factor', type=int, default=1, help='Max intensity of each trend anomaly')
    parser.add_argument('--variance_factor', type=int, default=0.5, help='Max intensity of each variance anomaly')
    parser.add_argument('--extreme_probability', type=float, default=0.3, help='Probability that an anomaly is of type extreme')
    parser.add_argument('--shift_probability', type=float, default=0.4, help='Probability that an anomaly is of type shift')
    parser.add_argument('--trend_probability', type=float, default=0.3, help='Probability that an anomaly is of type trend')
    parser.add_argument('--variance_probability', type=float, default=0.3, help='Probability that an anomaly is of type variance')
