...
# import sys																							   # Added
# print(sys.path)
# sys.path.insert(0, '/home/jonas/Documents/Time_Series_Generation_Benchmark/tsgen/lib/tensorflow-1.4.0.data/purelib')                    # Added
# print(sys.path)
...
#        df = pd.read_csv(data_path, header=None, nrows= 2)
        ...
        num_cont = df.shape[1]
        del df
        data_length = file_len(data_path) - 1                          ## ADDED -1. For some reason, file_len returns one too much
        ...
        for shift in winShift:
            try:
                ...
                export_path = './results/' + Path(data_path).stem + '/'
                # \
                ...
                if bool_LSH:
                    import stream.hashing.lsh_main as lsh																								# Changed, added 'stream.'
                    import os
					
                    lsh = lsh.LSH_conca()
                    ori, generation = lsh.lsh_main(data_path, export_path + model_path, export_path, window, top_n,
                                                   sampling_size, conca_plot,
                                                   gen_ts_length=gen_ts_length, gen_ts_dim=gen_ts_dim)
                                                   
                    # Copy long ts to 'normal' output path
                    os.system(f"cp fake_long_complete.csv {export_path}")
